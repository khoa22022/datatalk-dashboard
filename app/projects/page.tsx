...
export default function ProjectsPage(){
 ...
 useEffect(()=>{(async()=>{ if(!supabase)return; const {data}=await supabase.auth.getSession(); const token=data.session?.access_token; if(!token){window.location.href='/login';return;} fetch(`${API}/api/projects`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json()).then(d=>Array.isArray(d)&&setProjects(d)).catch(()=>{}); })();},[]);
 async function create(){ if(!name) return; const session=await supabase?.auth.getSession(); const token=session?.data.session?.access_token; if(!token){window.location.href='/login';return;} const r=await fetch(`${API}/api/projects`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({name,domain})}); const p=await r.json(); if(p?.id){setProjects(v=>[p,...v]);setCreated(p);setName('');setDomain('');} }
