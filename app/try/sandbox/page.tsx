...
export default function TrackingSandbox(){
  ...
  const seed=async()=>{setBusy(true);try{await trialRequest("/api/try/seed",{method:"POST"});setLog(x=>[`Seeded realistic tracking scenario`,...x].slice(0,20));await refresh()}catch(e){setLog(x=>[`Seed error · ${e instanceof Error?e.message:"unknown"}`,...x].slice(0,20))}finally{setBusy(false)}};
  ...
  return <>
    <div className="page-head"><div><h1>{t("sandboxTitle")}</h1><p>{t("sandboxSubtitle")}</p></div><div className="toolbar"><span className="pill good"><CheckCircle2 size={13}/> {t("backendLive")}</span><button className="btn primary" onClick={seed} disabled={busy}><Zap size={15}/>{t("seedScenario")}</button><button className="btn outline" onClick={reset} disabled={busy}><RotateCcw size={15}/>{t("resetSandbox")}</button></div></div>
